...
# TODO: val_honest update
# TODO: objective_cont update

# objective_cont: outcome variableがcontinuous variable
# objective: binary objective
...
def ace(outcome, treatment):
    ...
    :return: ATE
    # REMARK
    outcome, treatmentのindexは対応
    """
    treat = treatment == 1 #Boolean, treatedならばtrue
    control = treatment == 0 #Boolean, non-treatedならばtrue
    ...
    mu1 = 0.0 # float
    mu0 = 0.0
    if yt.shape[0] != 0: # sample sizeがzero以上
        mu1 = np.mean(yt)
    if yc.shape[0] != 0:
        mu0 = np.mean(yc)
    if (yt.shape[0] < 0.1) | (yc.shape[0] < 0.1):
        mu1 = None

    try:
        return mu1 - mu0
    except:
         print("Oops! something wrong with respect to treatment assignmemnt.  Try again...")
...
class CausalTree:
    def __init__(self, cont=False, max_depth=-1, min_size=2, weight=0.5, seed=None, split_size=0.5, honest=False,
        ...
        self.cont = cont
        if cont:
            self.eval_func = self.objective_cont ### outcomeがcontinuous
        else:
        ...
        self.honest = honest
        self.val_honest = val_honest ### 別の手法
    ...
    def objective(self, train_outcome, train_treatment, val_outcome, val_treatment):
        ...
        outcome: the observed binary outcome vector
        ...
        train_mse = (1 - self.weight) * total_train * (train_effect ** 2) ## Measuring the goodness of fit
